...
from t5_soft_utils import (compute_metrics, convert_examples_to_features,
...
MODEL_CLASSES = {'roberta': (T5Config, T5ForConditionalGeneration, RobertaTokenizer)}
...
def train(args, train_dataset, prompt_model, tokenizer, optimizer):
    ...
    mytemplate = prompt_model.template
    prompt_model.train()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        results = evaluate(args, prompt_model, tokenizer, mytemplate, checkpoint=str(global_step))
...
def evaluate(args, model, tokenizer, template, checkpoint=None, prefix="", mode='dev'):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        if (mode == 'dev'):
            eval_dataset = load_and_cache_examples(args, eval_task, tokenizer, template, ttype='dev')
        elif (mode == 'test'):
            eval_dataset, instances = load_and_cache_examples(args, eval_task, tokenizer, template, ttype='test')
...
def main():
    ...
    if args.config_name:
        ...
        config = config_class.from_pretrained("models/T5-small/config.json")
    ...
    if os.path.exists(vocab):
        ...
        tokenizer = tokenizer_class.from_pretrained("models/T5-small/")
    ...
    if os.path.exists(checkpoint_last) and os.listdir(checkpoint_last):
        ...
        model = model_class.from_pretrained("models/T5-small/pytorch_model.bin", config=config)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            p_model = PromptForClassification(plm=model, template=mytemplate, verbalizer=myverbalizer, freeze_plm=False)
            p_model.to(args.device)
            result = evaluate(args, p_model, tokenizer, mytemplate, checkpoint=checkpoint, prefix=global_step)
